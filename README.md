...
**Решение проблем:**
```bash
# Если порт 8000 занят или есть проблемы с процессами
...
- `reminder_scheduler.py` - система ежедневных напоминаний
- `start_reminders.py` / `start_reminders.sh` / `start_reminders.bat` - запуск напоминаний
...
Бот поддерживает автоматические ежедневные напоминания с сообщениями поддержки.

### Настройка для пользователей:
1. Откройте профиль в боте (кнопка "👤 Профиль")
2. Используйте кнопки "🔔 Включить напоминание" / "🔕 Выключить напоминание"
3. По умолчанию напоминания включены для всех новых пользователей

### Как работают напоминания:
- **Время работы:** с 12:00 до 20:00 каждый день
- **Частота:** одно сообщение в час в рабочее время
- **Контент:** сообщения поддержки от других пользователей по порядку (от минимального Message ID)
- **Алгоритм:** каждый день система выбирает следующее сообщение по ID, когда закончатся - начинает сначала
...
### Логи напоминаний:
Все активности системы напоминаний записываются в файл `reminders.log`.
...
- 🔔 Ежедневные напоминания - автоматические сообщения поддержки
